...
import { AlertCircle, ArrowDownRight, ArrowUpRight, BarChart4, CheckCircle, Clock, DollarSign, Hash, Hourglass, TimerOff, Wallet } from 'lucide-react';
          ...
          {exercise.map((exerciseItem, index) => {
            ...
            return (
              <TableRow key={index} className="border-gray-800 hover:bg-gray-900">
                ...
                <TableCell className="text-gray-300">{walletAddressShortn( exerciseItem.derivativeToken)}</TableCell>
                <TableCell className="text-gray-300">${Number(exerciseItem.exercisePrice)}</TableCell>
                <TableCell>
                  <div className="text-gray-300">{getTimeRemaining(Number(exerciseItem.expiryTime))}</div>
                  <div className="text-xs text-gray-500 flex items-center mt-1">
                                  <div className="text-xs text-gray-500 flex items-center mt-1">
                      {Number(exerciseItem.expiryTime) < Math.floor(Date.now() / 1000) ? (
